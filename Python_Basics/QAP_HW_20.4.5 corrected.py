...
quantity_all = []
for order_num, orders_data in orders.items():
    price_all.append(orders_data['price'])
    quantity_all.append(orders_data['quantity'])
print(f'Средняя стоимость товаров в июле: {round(sum(price_all) / sum(quantity_all), 2)}')